...
class CRUDTask(CRUDBase[models.Task, schemas.TaskCreateCrud, schemas.TaskUpdateCrud]):
    def schema_to_model_create(self, db: Session, *, create_obj: schemas.TaskCreateCrud) -> models.Task:
        ...
        image_instances = crud.image_instance.get_multi_by_ids(db, ids=create_obj.image_instance_ids)
        labels_assignments = crud.label_assignment.get_multi_by_ids(
            db, ids=create_obj.label_assignment_ids
        )
        annotations = crud.annotation.get_multi_by_ids(db, ids=create_obj.annotation_ids)
